...
task("FeederWrapper.deploy", "Deploy a new FeederWrapper").setAction(async (taskArgs, { ethers }) => {
        ...
        async (
            {
                ...
                vault,
                assets,
            }: { feederWrapper: string; feeder: string; assets: string[]; vault: string;},
            { ethers },
        ) => {
            const [deployer] = await ethers.getSigners()
            await sendTx(
...
task("FeederWrapper.approveMulti", "Sets approvals for multiple tokens/a single spender")
    ...
    .setAction(async ({ feederWrapper, tokens, spender }: { feederWrapper: string; tokens: string[]; spender: string }, { ethers }) => {
        const [deployer] = await ethers.getSigners()
        await sendTx(
            FeederWrapper__factory.connect(feederWrapper, deployer),
