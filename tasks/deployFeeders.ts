...
task("deployFeeder", "Deploys a feeder pool")
...
task("deployFeeder-mainnet", "Deploys all the feeder pools and required contracts")
    .addOptionalParam("speed", "Defender Relayer speed param: 'safeLow' | 'average' | 'fast' | 'fastest'", "fast", types.string)
    .setAction(async (taskArgs, { ethers, network }) => {
...
task("deployIronBank", "Deploys mUSD Iron Bank (CREAM) integration contracts for GUSD and BUSD Feeder Pools")
    ...
    .setAction(async (taskArgs, hre) => {
        ...
        const deployer = await getSigner(hre.network.name, hre.ethers, taskArgs.speed)
