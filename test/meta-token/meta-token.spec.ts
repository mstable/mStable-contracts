...
import { ERC20, ImmutableModule, ERC20Burnable, MetaToken, MetaToken__factory, Nexus, Nexus__factory } from "types/generated"
...
describe("MetaToken", () => {
    ...
    describe("verifying ERC20 properties", async () => {
        ...
        shouldBehaveLikeERC20(ctxERC20 as IERC20BehaviourContext, "ERC20", simpleToExactAmount(100000000, 18))

        shouldBehaveLikeERC20Burnable(ctxERC20Burnable as IERC20BurnableBehaviourContext, "ERC20", simpleToExactAmount(100000000, 18))
    ...
    describe("custom ERC20Mintable", async () => {
        ...
        describe("managing minters", () => {
            it("should not allow minters to add minters", async () => {
                await expect(meta.connect(sa.dummy1.signer).addMinter(sa.dummy2.address)).to.be.revertedWith("Only governor can execute")
                await expect(meta.connect(sa.dummy2.signer).addMinter(sa.dummy2.address)).to.be.revertedWith("Only governor can execute")
            })
            ...
            it("should not allow minters to remove minters", async () => {
                ...
                await expect(meta.connect(sa.dummy1.signer).removeMinter(sa.dummy1.address)).to.be.revertedWith("Only governor can execute")
                await expect(meta.connect(sa.dummy2.signer).removeMinter(sa.dummy1.address)).to.be.revertedWith("Only governor can execute")
            })
            ...
            it("should allow a minter to renounce their minting ability", async () => {
                ...
                await expect(meta.connect(sa.dummy1.signer).renounceMinter()).to.be.revertedWith(
        ...
        describe("minting Meta", () => {
            it("should not allow a EOA to mint", async () => {
                await expect(meta.connect(sa.default.signer).mint(sa.dummy1.address, 1)).to.be.revertedWith(
                    "MinterRole: caller does not have the Minter role",
                )
            })
            it("should not allow the governor to mint directly", async () => {
                await expect(meta.connect(sa.governor.signer).mint(sa.dummy1.address, 1)).to.be.revertedWith(
                    "MinterRole: caller does not have the Minter role",
                )
            })
            ...
            it("should not allow a removed minter", async () => {
                ...
                await expect(meta.connect(sa.dummy1.signer).mint(sa.dummy1.address, 1)).to.be.revertedWith(
                    "MinterRole: caller does not have the Minter role",
                )
            })
        })
