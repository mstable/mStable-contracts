...
contract("TestMassetTokenTransferable", accounts => {
	...
	it("The total supply is correctly initialized.", async () => {
		let instance = await TestMassetTokenTransferable.deployed();
		let test_user = await instance.init_total_supply.call({from: user});
		assert.equal(test_user, true, "Check the constructor of TestMassetTokenTransferable");
	});
	...
	it("All the users have a positive balance.", async () => {
		...
		assert.equal(test_owner, true, "Check the constructor of TestMassetTokenTransferable");
		...
		assert.equal(test_attacker, true, "Check the constructor of TestMassetTokenTransferable");
	});
