...
export function shouldBehaveLikeERC20Burnable(
    ctx: IERC20BurnableBehaviourContext,
    errorPrefix: string,
    balance: BN,
): void {
    describe("burn", () => {
        describe("when the given amount is not greater than balance of the sender", () => {
            ...
            const shouldBurn = (amount) => {
                ...
                it("burns the requested amount", async () => {
                    expect(await ctx.burnableToken.balanceOf(ctx.owner.address)).eq(
                        balance.sub(amount),
                    );
                });

                it("emits a transfer event", async () => {
                    await expect(tx).to.emit(ctx.burnableToken, "Transfer")
                        .withArgs(ctx.owner.address, ZERO_ADDRESS, amount);
                });
            };
        ...
        describe("on success", () => {
            const shouldBurnFrom = (amount) => {
                ...
                it("decrements allowance", async () => {
                    expect(await ctx.burnableToken.allowance(ctx.owner.address, ctx.burner.address)).eq(
