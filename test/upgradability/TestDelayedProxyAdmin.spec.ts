...
contract("DelayedProxyAdmin", async (accounts) => {
    ...
    describe("", async () => {
        beforeEach("before each", async () => {
            ...
            const data = mockImplV1.contract.methods
                .initialize(delayedProxyAdmin.address)
                .encodeABI();
            ...
            describe("getProxyImplementation()", async () => {
                context("should succeed", async () => {
                    it("when proxy exist and returns implementation address", async () => {
                        const implAddr = await delayedProxyAdmin.getProxyImplementation(
                            proxy.address,
                        );
                    ...
                    it("when proxy upgraded to new implementation", async () => {
                        let implAddr = await delayedProxyAdmin.getProxyImplementation(
                            proxy.address,
                        );
                        expect(mockImplV1.address).to.equal(implAddr);
