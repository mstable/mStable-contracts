...
library MassetManagerV4 {
    ...
    function migrateBassets(
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            // 4. Deposit everything into the new integration
    ...
    ImmutableModule
{
    ...
    function _mintMulti(
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            if (bAssetQuantity > 0) {
                ...
                uint256 quantityDeposited =
                    MassetManagerV4.depositTokens(personal, data.ratio, bAssetQuantity, cache.maxCache);
    ...
    ****************************************/
    ...
    function _swap(
        ...
        MassetManagerV4.withdrawTokens(
            swapOutput,
            outputPersonal,
    ...
    ****************************************/
        ...
        returns (uint256 mintAmount, uint256 newSupply)
    {
        ...
        (mintAmount, gains) = MassetManagerV4.collectPlatformInterest(
            bAssetPersonal,
            bAssetData,
            forgeValidator,
            _getConfig()
    ...
    ****************************************/
    ...
    function setTransferFeesFlag(address _bAsset, bool _flag) external override onlyGovernor {
        MassetManagerV4.setTransferFeesFlag(bAssetPersonal, bAssetIndexes, _bAsset, _flag);
    }
    ...
    function migrateBassets(address[] calldata _bAssets, address _newIntegration)
        ...
        onlyGovernor
    {
        MassetManagerV4.migrateBassets(bAssetPersonal, bAssetIndexes, _bAssets, _newIntegration);
    }
    ...
    function handlePegLoss(address _bAsset, bool _belowPeg) external onlyGovernor {
        MassetManagerV4.handlePegLoss(basket, bAssetPersonal, bAssetIndexes, _bAsset, _belowPeg);
    }
    ...
    function negateIsolation(address _bAsset) external onlyGovernor {
        MassetManagerV4.negateIsolation(basket, bAssetPersonal, bAssetIndexes, _bAsset);
    }
    ...
    function startRampA(uint256 _targetA, uint256 _rampEndTime) external onlyGovernor {
        MassetManagerV4.startRampA(ampData, _targetA, _rampEndTime, _getA(), A_PRECISION);
    }
    ...
    function stopRampA() external onlyGovernor {
        MassetManagerV4.stopRampA(ampData, _getA());
    }
}