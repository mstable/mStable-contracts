    ...
    InitializableReentrancyGuard
{
    ...
    ****************************************/
        ...
        returns (uint256 mintAmount, uint256 newSupply)
    {
        (uint8[] memory idxs, uint256[] memory gains) =
            FeederManagerV2.calculatePlatformInterest(data.bAssetPersonal, data.bAssetData);
    ...
    ****************************************/
    ...
    function migrateBassets(address[] calldata _bAssets, address _newIntegration)
        ...
        onlyGovernor
    {
        FeederManagerV2.migrateBassets(data.bAssetPersonal, _bAssets, _newIntegration);
    }
    ...
    function startRampA(uint256 _targetA, uint256 _rampEndTime) external onlyGovernor {
        FeederManagerV2.startRampA(data.ampData, _targetA, _rampEndTime, _getA(), A_PRECISION);
    }
    ...
    function stopRampA() external onlyGovernor {
        FeederManagerV2.stopRampA(data.ampData, _getA());
    }

    /**
     * @notice Withdraws some liquid bAssets from the lending platform to the cache in the integration contract.
     * @param _bAsset Address of the bAsset that is being withdrawn from the lending platform. eg GUSD, BUSD, mUSD.
     * @param _withdrawAmount Amount of bAsset to withdraw from the lending platform
     */
    function cachePlatformIntegration(address _bAsset, uint256 _withdrawAmount) external {

        Asset memory asset = _getAsset(_bAsset);
        require(asset.exists, "Invalid asset");
        BassetPersonal memory _personal = data.bAssetPersonal[asset.idx];
