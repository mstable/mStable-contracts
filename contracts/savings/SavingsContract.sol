...
// Internal
import { ISavingsContractV3, ISavingsContractV2 } from "../interfaces/ISavingsContract.sol";
...
contract SavingsContract is ISavingsContractV3, Initializable, InitializableToken, ImmutableModule {
    ...
    // Should it be immutable or set by governance?
    // Should it be on Nexus?
    //address unwrapper = nexus.getModule(keccak256("Unwrapper"));
    address private constant unwrapper = 0x0000000000000000000000000000000000000000;
    ...
    ****************************************/
    ...
    function redeemAndUnwrap(
        ...
        // Ensure that the payout was sufficient
        (uint256 credits, uint256 massetReturned) = _redeem(_underlying, false, false);
        require(massetReturned == _underlying, "Invalid output");

        // Approve wrapper to spend contract's underlying; just for this tx
        // TODO set unwrapper address somewhere (Nexus?)
        underlying.approve(address(unwrapper), _underlying);

        // Unwrap the underlying into `output` and transfer to `beneficiary`
        unwrapper.unwrapAndSend(
            address(underlying),
            _output,
            _underlying,
            _minAmountOut,
