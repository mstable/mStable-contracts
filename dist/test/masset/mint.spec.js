...
describe("Masset - Mint", () => {
    ...
    let mAssetMachine;
    let details;
    const runSetup = async (seedBasket = true, useTransferFees = false, useLendingMarkets = false) => {
    ...
    const assertFailedMint = async (expectedReason, mAssetContract, bAsset, bAssetQuantity, minMassetQuantity = 0, approval = true, sender = sa.default.signer, recipient = sa.default.address, mintOutputRevertExpected = true, mintOutputExpected = 0, quantitiesAreExact = false) => {
        ...
        const bAssetQuantityExact = quantitiesAreExact ? math_1.BN.from(bAssetQuantity) : math_1.simpleToExactAmount(bAssetQuantity, bAssetDecimals);
        const minMassetQuantityExact = quantitiesAreExact ? math_1.BN.from(minMassetQuantity) : math_1.simpleToExactAmount(minMassetQuantity, 18);
        await chai_1.expect(mAsset.mint(bAsset.address, bAssetQuantityExact, minMassetQuantityExact, recipient), `mint tx should revert with "${expectedReason}"`).to.be.revertedWith(expectedReason);
        if (mintOutputRevertExpected) {
            await chai_1.expect(mAsset.getMintOutput(bAsset.address, bAssetQuantityExact), `getMintOutput call should revert with "${expectedReason}"`).to.be.revertedWith(expectedReason);
        }
        else {
            const mintOutputExpectedExact = quantitiesAreExact ? math_1.BN.from(mintOutputExpected) : math_1.simpleToExactAmount(mintOutputExpected, 18);
            const output = await mAsset.getMintOutput(bAsset.address, bAssetQuantityExact);
            chai_1.expect(output, "getMintOutput call output").eq(mintOutputExpectedExact);
        }
    };
    const assertFailedMintMulti = async (expectedReason, mAssetContract, bAssets, bAssetRedeemQuantities, minMassetQuantity = 0, approval = true, sender = sa.default.signer, recipient = sa.default.address, mintMultiOutputRevertExpected = true, outputExpected = 0, quantitiesAreExact = false) => {
        const mAsset = mAssetContract.connect(sender);
        if (approval) {
            const approvePromises = bAssets.map((b, i) => typeof b === "string"
        ...
        const minMassetQuantityExact = quantitiesAreExact ? math_1.BN.from(minMassetQuantity) : math_1.simpleToExactAmount(minMassetQuantity, 18);
        await chai_1.expect(mAsset.mintMulti(bAssetAddresses, bAssetRedeemQuantitiesExact, minMassetQuantityExact, recipient), `mintMulti tx should revert with "${expectedReason}"`).to.be.revertedWith(expectedReason);
        if (mintMultiOutputRevertExpected) {
            await chai_1.expect(mAsset.getMintMultiOutput(bAssetAddresses, bAssetRedeemQuantitiesExact), `getMintMultiOutput call should revert with "${expectedReason}"`).to.be.revertedWith(expectedReason);
        }
        else {
            ...
            chai_1.expect(output, "getMintMultiOutput call output").eq(outputExpectedExact);
        }
    };
    // Helper to assert basic minting conditions, i.e. balance before and after
    const assertBasicMint = async (md, bAsset, bAssetQuantity, minMassetQuantity = 0, recipient = sa.default.address, sender = sa.default, ignoreHealthAssertions = true, quantitiesAreExact = false) => {
        const { platform } = md;
        ...
        const bAssetQuantityExact = quantitiesAreExact
            ? math_1.BN.from(bAssetQuantity)
            : math_1.simpleToExactAmount(bAssetQuantity, await bAsset.decimals());
        const minMassetQuantityExact = quantitiesAreExact ? math_1.BN.from(minMassetQuantity) : math_1.simpleToExactAmount(minMassetQuantity, 18);
        const mAssetQuantityExact = math_1.applyRatio(bAssetQuantityExact, bAssetBefore.ratio);
        const platformInteraction = await machines_1.MassetMachine.getPlatformInteraction(mAsset, "deposit", bAssetQuantityExact, bAssetBefore);
        ...
        assertions_1.assertBNClosePercent(mAssetOutput, mAssetQuantityExact, "0.02", "mAssetOutput");
        ...
        chai_1.expect(integratorBalAfter, "integratorBalAfter").eq(integratorBalBefore.add(bAssetQuantityExact));
        if (platformInteraction.expectInteraction) {
            await chai_1.expect(tx).to.emit(platform, "Deposit").withArgs(bAsset.address, bAssetBefore.pToken, platformInteraction.amount);
        }
        // Recipient should have mAsset quantity after
        const recipientBalAfter = await mAsset.balanceOf(recipient);
        chai_1.expect(recipientBalAfter, "recipientBal after").eq(recipientBalBefore.add(mAssetOutput));
        // Sender should have less bAsset after
        const senderBassetBalAfter = await bAsset.balanceOf(sender.address);
        chai_1.expect(senderBassetBalAfter, "senderBassetBal after").eq(senderBassetBalBefore.sub(bAssetQuantityExact));
        // VaultBalance should update for this bAsset
        const bAssetAfter = await mAsset.getBasset(bAsset.address);
        chai_1.expect(math_1.BN.from(bAssetAfter.bData.vaultBalance), "vaultBalance after").eq(math_1.BN.from(bAssetBefore.vaultBalance).add(bAssetQuantityExact));
        ...
        context("when the weights are within the ForgeValidator limit", () => {
            ...
            context("using bAssets with transfer fees", async () => {
                ...
                it("should handle tokens with transfer fees", async () => {
                    ...
                    chai_1.expect(recipientBalBefore).eq(0);
                    const mAssetMintAmount = 10;
                    const approval0 = await mAssetMachine.approveMasset(bAsset, mAsset, mAssetMintAmount);
                    // 3.0 Do the mint
                    const tx = mAsset.mint(bAsset.address, approval0, 0, recipient.address);
                    const mAssetQuantity = math_1.simpleToExactAmount(mAssetMintAmount, 18);
                    const bAssetQuantity = math_1.simpleToExactAmount(mAssetMintAmount, await bAsset.decimals());
                    ...
                    await chai_1.expect(tx).to.emit(bAsset, "Transfer").withArgs(sa.default.address, platform.address, bAssetAmountLessFee);
                    ...
                    assertions_1.assertBNSlightlyGTPercent(recipientBalBefore.add(mAssetQuantity), recipientBalAfter, "0.3", true);
                    // Sender should have less bAsset after
                    const minterBassetBalAfter = await bAsset.balanceOf(sa.default.address);
                    chai_1.expect(minterBassetBalAfter, "minterBassetBalAfter").eq(minterBassetBalBefore.sub(bAssetQuantity));
                });
                it("should fail if the token charges a fee but we don't know about it", async () => {
                    ...
                    const approval0 = await mAssetMachine.approveMasset(bAsset, mAsset, mAssetMintAmount);
                    // 3.0 Do the mint
                    await chai_1.expect(mAsset.mint(bAsset.address, approval0, 0, sa.default.address)).to.revertedWith("Asset not fully transferred");
                });
            });
            context("with an affected bAsset", async () => {
                it("should fail if bAsset is broken below peg", async () => {
                    ...
                    await assertFailedMint("Unhealthy", mAsset, bAsset, "1000000000000000000", 0, true, sa.default.signer, sa.default.address, false, "1000746841283429855", true);
                });
            });
            context("passing invalid arguments", async () => {
                ...
                it("should fail if recipient is 0x0", async () => {
                    ...
                    await assertFailedMint("Invalid recipient", mAsset, bAssets[0], "1000000000000000000", 0, true, sa.default.signer, constants_1.ZERO_ADDRESS, false, "999854806326923450", true);
                });
                ...
                it("should fail if sender doesn't have balance", async () => {
                    ...
                    await assertFailedMint("ERC20: transfer amount exceeds balance", mAsset, bAsset, "100000000000000000000", "99000000000000000000", true, sender.signer, sender.address, false, "98939327585405193936", true);
                });
                it("should fail if sender doesn't give approval", async () => {
                    ...
                    chai_1.expect(await bAsset.allowance(sender.address, mAsset.address)).eq(0);
                    chai_1.expect(await bAsset.balanceOf(sender.address)).eq(10000);
                    await assertFailedMint("ERC20: transfer amount exceeds allowance", mAsset, bAsset, 100, 99, false, sender.signer, sender.address, false, 100, true);
                });
    ...
    describe("minting with multiple bAssets", () => {
        // Helper to assert basic minting conditions, i.e. balance before and after
        const assertMintMulti = async (md, mAssetMintAmounts, bAssets, recipient = sa.default.address, sender = sa.default, ignoreHealthAssertions = false) => {
            const { mAsset } = md;
            if (!ignoreHealthAssertions)
                await assertions_1.assertBasketIsHealthy(mAssetMachine, md);
            ...
            const approvals = await Promise.all(bAssets.map((b, i) => mAssetMachine.approveMasset(b, mAsset, mAssetMintAmounts[i])));
            const mAssetOutput = await mAsset.getMintMultiOutput(bAssetBefore.map((b) => b.personal.addr), approvals);
            const mAssetQuantity = math_1.simpleToExactAmount(mAssetMintAmounts.reduce((p, c) => math_1.BN.from(p).add(math_1.BN.from(c)), math_1.BN.from(0)), 18);
            assertions_1.assertBNClosePercent(mAssetOutput, mAssetQuantity, "0.25", "mAssetOutput");
            const tx = mAsset.connect(sender.signer).mintMulti(bAssetBefore.map((b) => b.personal.addr), approvals, 0, recipient);
            await chai_1.expect(tx)
                .to.emit(mAsset, "MintedMulti")
                .withArgs(sender.address, recipient, mAssetOutput, bAssetBefore.map((b) => b.personal.addr), approvals);
            const bAssetQuantities = mAssetMintAmounts.map((m, i) => math_1.simpleToExactAmount(m, bAssetDecimals[i]));
            // Recipient should have mAsset quantity after
            const recipientBalAfter = await mAsset.balanceOf(recipient);
            chai_1.expect(recipientBalAfter, "recipientBalAfter").eq(recipientBalBefore.add(mAssetOutput));
            // Sender should have less bAsset after
            const minterBassetBalAfter = await Promise.all(bAssets.map((b) => b.balanceOf(sender.address)));
            minterBassetBalAfter.map((b, i) => chai_1.expect(b, `minter bAsset ${i} bal`).eq(minterBassetBalBefore[i].sub(bAssetQuantities[i])));
            // VaultBalance should updated for this bAsset
            const bAssetAfter = await Promise.all(bAssets.map((b) => mAsset.getBasset(b.address)));
            bAssetAfter.map((b, i) => chai_1.expect(b.bData.vaultBalance, `vault balance ${i}`).eq(math_1.BN.from(bAssetBefore[i].bData.vaultBalance).add(bAssetQuantities[i])));
            // Complete basket should remain in healthy state
            if (!ignoreHealthAssertions)
                await assertions_1.assertBasketIsHealthy(mAssetMachine, md);
        };
        ...
        context("when the weights are within the ForgeValidator limit", () => {
            context("and sending to a specific recipient", async () => {
                ...
                it("should mint selected bAssets only", async () => {
                    ...
                    chai_1.expect(compBefore.bAssets[1].vaultBalance).eq(compAfter.bAssets[1].vaultBalance);
                    chai_1.expect(compBefore.bAssets[3].vaultBalance).eq(compAfter.bAssets[3].vaultBalance);
                });
                it("should send mUSD when recipient is a contract", async () => {
                    const { bAssets, managerLib } = details;
                    const recipient = managerLib.address;
            ...
            context("and specifying one bAsset base unit", async () => {
                ...
                it("should mint a higher q of mAsset base units when using bAsset with 18", async () => {
                    ...
                    const expectedMasset = math_1.BN.from(10).pow(math_1.BN.from(18).sub(decimals));
                    await chai_1.expect(tx)
                        .to.emit(mAsset, "MintedMulti")
                    ...
                    chai_1.expect(recipientBalAfter).eq(recipientBalBefore.add(expectedMasset));
                    // Sender should have less bAsset after
                    const minterBassetBalAfter = await bAsset.balanceOf(sa.default.address);
                    chai_1.expect(minterBassetBalAfter).eq(minterBassetBalBefore.sub(1));
                    // Complete basket should remain in healthy state
                    await assertions_1.assertBasketIsHealthy(mAssetMachine, details);
                });
            });
            context("using bAssets with transfer fees", async () => {
                ...
                it("should handle tokens with transfer fees", async () => {
                    ...
                    await chai_1.expect(tx).to.emit(bAsset, "Transfer").withArgs(platform.address, lendingPlatform, bAssetAmountLessFee.mul(999).div(1000));
                    // 3.3 Check Deposits into lending platform
                    await chai_1.expect(tx).to.emit(platform, "Deposit").withArgs(bAsset.address, platformToken, bAssetAmountLessFee);
                    // 4.0 Recipient should have mAsset quantity after
                    const recipientBalAfter = await mAsset.balanceOf(recipient.address);
                    // Assert that we minted gt 99% of the bAsset
                    assertions_1.assertBNSlightlyGTPercent(recipientBalBefore.add(mAssetQuantity), recipientBalAfter, "0.3");
                    // Sender should have less bAsset after
                    const minterBassetBalAfter = await bAsset.balanceOf(sa.default.address);
                    chai_1.expect(minterBassetBalAfter, "minterBassetBalAfter").eq(minterBassetBalBefore.sub(bAssetQuantity));
                    // Complete basket should remain in healthy state
                ...
                it("should fail if the token charges a fee but we don't know about it", async () => {
                    ...
                    const approval0 = await mAssetMachine.approveMasset(bAsset, mAsset, mAssetMintAmount);
                    // 3.0 Do the mint
                    await chai_1.expect(mAsset.mintMulti([bAsset.address], [approval0], 0, sa.default.address)).to.revertedWith("Asset not fully transferred");
                });
            });
            context("with an affected bAsset", async () => {
                it("should fail if bAsset is broken below peg", async () => {
                    ...
                    await assertFailedMintMulti("Unhealthy", mAsset, [bAsset.address], ["1000000000000000000"], 0, true, sa.default.signer, sa.default.address, false, "1000746841283429855", true);
                });
            });
            context("passing invalid arguments", async () => {
                before(async () => {
                ...
                it("should fail if recipient is 0x0", async () => {
                    const { mAsset, bAssets } = details;
                    await assertFailedMintMulti("Invalid recipient", mAsset, [bAssets[0].address], [1], 0, true, sa.default.signer, constants_1.ZERO_ADDRESS, false, 1, true);
                });
                ...
                describe("minting with some 0 quantities", async () => {
                    ...
                    it("should fail if output mAsset quantity is 0", async () => {
                        ...
                        // Pass all 0's
                        await assertFailedMintMulti("Zero mAsset quantity", mAsset, bAssetBefore.map((b) => b.personal.addr), [0, 0, 0, 0], 0, true, sa.default.signer, sa.default.address, false, 0);
                    });
                });
                it("should fail if slippage just too big", async () => {
                    ...
                    await assertFailedMintMulti("Mint quantity < min qty", mAsset, [bAsset.address], ["100000000000000000000"], // 100
                    "100000000000000000001", // just over 100
                    true, sender.signer, sender.address, false, "98939327585405193936", // 0.989...
                    true);
                });
                it("should fail if sender doesn't have balance", async () => {
                    const { bAssets, mAsset } = details;
                    ...
                    const sender = sa.dummy2;
                    chai_1.expect(await bAsset.balanceOf(sender.address)).eq(0);
                    await assertFailedMintMulti("ERC20: transfer amount exceeds balance", mAsset, [bAsset.address], ["100000000000000000000"], 0, false, sender.signer, sender.address, false, "98939327585405193936", true);
                });
                it("should fail if sender doesn't give approval", async () => {
                    const { bAssets, mAsset } = details;
                    const bAsset = bAssets[0];
                    ...
                    chai_1.expect(await bAsset.allowance(sender.address, mAsset.address)).eq(0);
                    chai_1.expect(await bAsset.balanceOf(sender.address)).eq(10000);
                    await assertFailedMintMulti("ERC20: transfer amount exceeds allowance", mAsset, [bAsset.address], [100], 0, false, sender.signer, sa.default.address, false, 100, true);
                });
                it("should fail if the bAsset does not exist", async () => {
                    const { mAsset } = details;
            ...
            describe("minting with various orders", async () => {
                ...
                it("should mint quantities relating to the order of the bAsset indexes", async () => {
                    ...
                    const compAfter = await mAssetMachine.getBasketComposition(details);
                    chai_1.expect(compAfter.bAssets[0].vaultBalance).eq(math_1.BN.from(compBefore.bAssets[0].vaultBalance).add(math_1.BN.from(2)));
                    chai_1.expect(compAfter.bAssets[1].vaultBalance).eq(math_1.BN.from(compBefore.bAssets[1].vaultBalance).add(math_1.BN.from(1)));
                });
                it("should mint using multiple bAssets", async () => {
                    ...
                    // low decimal bAssets
                    const approvals = await mAssetMachine.approveMassetMulti([bAssets[0], bAssets[1], bAssets[2]], mAsset, 1, sa.default.signer);
                    await mAsset.mintMulti([bAssets[0].address, bAssets[1].address, bAssets[2].address], approvals, 0, sa.default.address);
                    const approvals2 = await mAssetMachine.approveMassetMulti([bAssets[0], bAssets[1], bAssets[2], bAssets[3]], mAsset, 1, sa.default.signer);
                    const mUsdBalBefore = await mAsset.balanceOf(sa.default.address);
                    await mAsset.mintMulti([bAssets[0].address, bAssets[1].address, bAssets[2].address, bAssets[3].address], approvals2, 0, sa.default.address);
                    const mUsdBalAfter = await mAsset.balanceOf(sa.default.address);
                    assertions_1.assertBNClosePercent(mUsdBalAfter, mUsdBalBefore.add(math_1.simpleToExactAmount(4, 18)), "0.0001", "Must mint 4 full units of mUSD");
                });
        ...
        context("when the mAsset is undergoing re-collateralisation", () => {
            ...
            it("should revert any mints", async () => {
                ...
                await chai_1.expect(mAsset.mintMulti([bAsset0.address], [1], 0, sa.default.address)).to.revertedWith("Unhealthy");
            });
        });
    });
});
