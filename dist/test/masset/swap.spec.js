...
describe("Masset - Swap", () => {
     ...
     */
    const assertFailedSwap = async (mAssetContract, inputBasset, outputAsset, inputQuantity, minOutputQuantity, expectedReason, sender = sa.default.signer, recipient = sa.default.address, callSwapOutput = true, swapOutputRevertExpected = false, swapOutputExpected = 0, quantitiesAreExact = false) => {
        const mAsset = mAssetContract.connect(sender);
        const approval = await mAssetMachine.approveMasset(inputBasset, mAsset, inputQuantity, sa.default.signer, quantitiesAreExact);
        ...
        await chai_1.expect(mAsset.swap(inputBasset.address, outputAsset.address, approval, minOutputQuantityExact, recipient), `swap tx should revert with "${expectedReason}"`).to.be.revertedWith(expectedReason);
        ...
        if (callSwapOutput) {
            if (swapOutputRevertExpected) {
                await chai_1.expect(mAsset.getSwapOutput(inputBasset.address, outputAsset.address, approval), `getSwapOutput call should revert with "${expectedReason}"`).to.be.revertedWith(expectedReason);
            }
            else {
                const swapOutputExpectedExact = quantitiesAreExact
                    ? math_1.BN.from(swapOutputExpected)
                    : math_1.simpleToExactAmount(swapOutputExpected, outputDecimals);
                const output = await mAsset.getSwapOutput(inputBasset.address, outputAsset.address, approval);
                chai_1.expect(output, "getSwapOutput call output").eq(swapOutputExpectedExact);
            }
        }
    };
    /**
     ...
     */
    const assertSwap = async (md, inputBasset, outputAsset, inputQuantity, recipient = sa.default.address, sender = sa.default, ignoreHealthAssertions = false, swapQuantityIsExact = false, minOutputQuantity = 1) => {
        const { platform } = md;
        ...
        const approval0 = await mAssetMachine.approveMasset(inputBasset, mAsset, math_1.BN.from(inputQuantity), sender.signer, swapQuantityIsExact);
        ...
        const platformInteractionIn = await machines_1.MassetMachine.getPlatformInteraction(mAsset, "deposit", approval0, inputBassetBefore);
        const platformInteractionOut = await machines_1.MassetMachine.getPlatformInteraction(mAsset, "withdrawal", expectedOutputValue, outputBassetBefore);
        ...
        // Swapped event
        await chai_1.expect(swapTx).to.emit(mAsset, "Swapped");
        ...
        chai_1.expect(swappedEvent.args[0]).to.eq(sender.address);
        chai_1.expect(swappedEvent.args[1]).to.eq(inputBasset.address);
        chai_1.expect(swappedEvent.args[2]).to.eq(outputAsset.address);
        chai_1.expect(swappedEvent.args[3]).to.eq(expectedOutputValue);
        const scaledFee = swappedEvent.args[4];
        chai_1.expect(swappedEvent.args[5]).to.eq(recipient);
        // Input Transfer event
        await chai_1.expect(swapTx, "Transfer event for input bAsset from sender to platform integration or mAsset")
        ...
        await chai_1.expect(swapTx, "Transfer event for output bAsset from platform integration or mAsset to recipient")
        ...
        await swapTx;
        const inputIntegratorBalAfter = await inputBassetBefore.contract.balanceOf(inputBassetBefore.integrator ? inputBassetBefore.integratorAddr : mAsset.address);
        chai_1.expect(inputIntegratorBalAfter, "Input destination raw balance").eq(platformInteractionIn.rawBalance);
        const outputIntegratorBalAfter = await outputBassetBefore.contract.balanceOf(outputBassetBefore.integrator ? outputBassetBefore.integratorAddr : mAsset.address);
        ...
        chai_1.expect(swapperBassetBalAfter, "swapperBassetBalAfter incorrect").eq(swapperInputBalBefore.sub(inputQuantityExact));
        //    VaultBalance should update for input bAsset
        const inputBassetAfter = await mAsset.getBasset(inputBasset.address);
        chai_1.expect(math_1.BN.from(inputBassetAfter.bData.vaultBalance), "inputBassetAfter incorrect").eq(math_1.BN.from(inputBassetBefore.vaultBalance).add(inputQuantityExact));
        //  Output
        //    Lending market
        if (platformInteractionOut.expectInteraction) {
            ...
            await chai_1.expect(swapTx).to.emit(platform, "Withdrawal").withArgs(outputAsset.address, constants_1.ZERO_ADDRESS, expectedOutputValue);
        }
        //    Recipient should have output asset quantity after (minus fee)
        const recipientBalAfter = await outputAsset.balanceOf(recipient);
        chai_1.expect(recipientBalAfter, "recipientBalAfter incorrect").eq(recipientOutputBalBefore.add(expectedOutputValue));
        //    Swap estimation should match up
        chai_1.expect(expectedOutputValue, "expectedOutputValue incorrect").eq(recipientBalAfter.sub(recipientOutputBalBefore));
        //    VaultBalance should update for output bAsset
        const outputBassetAfter = await mAsset.getBasset(outputAsset.address);
        chai_1.expect(math_1.BN.from(outputBassetAfter.bData.vaultBalance), "outputBassetAfter incorrect").eq(math_1.BN.from(outputBassetBefore.vaultBalance).sub(expectedOutputValue));
        // Global
        //   Fees should accrue to surplus
        const { surplus: surplusAfter } = await mAsset.data();
        chai_1.expect(math_1.BN.from(surplusAfter), "surplusAfter incorrect").eq(math_1.BN.from(surplusBefore).add(scaledFee));
        if (!ignoreHealthAssertions)
            await assertions_1.assertBasketIsHealthy(mAssetMachine, md);
    ...
    describe("swapping assets", () => {
        context("when within the invariant validator limits", () => {
            ...
            it("should fail if min qty < output qty", async () => {
                ...
                await assertFailedSwap(mAsset, bAssets[2], // wBTC 12 decimal places
                bAssets[1], // sBTC 6 decimal places
                math_1.simpleToExactAmount(5, 12), // sBTC input qty
                math_1.simpleToExactAmount(4.986727, 6), // min output qty = 0.06% of input + a bit
                "Output qty < minimum qty", sa.default.signer, sa.default.address, false, false, undefined, true);
            });
            it("should swap with min qty same as qty less swap fee", async () => {
                ...
                const inputQty = math_1.simpleToExactAmount(5, 12); // wBTC input qty
                const expectedOutputQty = math_1.simpleToExactAmount(4.986726, 6); // min output qty ~= 0.06% of input
                const outputQty = await mAsset.getSwapOutput(inputBasset.address, outputBasset.address, inputQty);
                chai_1.expect(outputQty, "incorrect swap output quantity").to.eq(expectedOutputQty);
                await assertSwap(details, inputBasset, outputBasset, inputQty, undefined, undefined, undefined, true, // exact units
                expectedOutputQty);
            });
            ...
            context("and specifying one bAsset base unit", async () => {
                ...
                it("should fail if output has less decimals", async () => {
                    ...
                    chai_1.expect(await input.decimals()).eq(18);
                    chai_1.expect(await output.decimals()).eq(6);
                    await assertFailedSwap(mAsset, input, output, 1, 0, "Zero swap output", sa.default.signer, sa.default.address, false, true, undefined, true);
                });
                it("should swap a higher q of bAsset base units if output has more decimals", async () => {
                    ...
                    chai_1.expect(await input.decimals()).eq(6);
                    chai_1.expect(await output.decimals()).eq(18);
            ...
            context("passing invalid arguments", async () => {
                ...
                it("should fail when 0 quantity", async () => {
                    const { bAssets, mAsset } = details;
                    await assertFailedSwap(mAsset, bAssets[0], bAssets[1], 0, 0, "Invalid swap quantity", sa.default.signer, sa.default.address, true, true);
                });
                it("should fail if recipient is 0x0", async () => {
                    const { bAssets, mAsset } = details;
                    await assertFailedSwap(mAsset, bAssets[0], bAssets[1], 1, 0, "Invalid recipient", sa.default.signer, constants_1.ZERO_ADDRESS, true, false, 0.999003);
                });
                it("should fail if sender doesn't have sufficient liquidity", async () => {
                    const { bAssets, mAsset } = details;
                    await assertFailedSwap(mAsset, bAssets[0], bAssets[1], 1, 0, "ERC20: transfer amount exceeds balance", sa.dummy1.signer, sa.dummy1.address, true, false, 0.999003);
                });
                it("should fail if sender doesn't give approval", async () => {
                    ...
                    chai_1.expect(await input.allowance(sender.address, mAsset.address)).eq(0);
                    chai_1.expect(await input.balanceOf(sender.address)).eq(10000);
                    await chai_1.expect(mAsset.connect(sender.signer).swap(input.address, bAssets[1].address, 5, 1, sa.default.address)).to.revertedWith("ERC20: transfer amount exceeds allowance");
                });
                it("should fail if *either* bAsset does not exist", async () => {
                    ...
                    await chai_1.expect(mAsset.swap(fakeBasset, realBasset, 1, 0, recipient)).to.revertedWith(expectedReason);
                    await chai_1.expect(mAsset.swap(realBasset, fakeBasset, 1, 0, recipient)).to.revertedWith(expectedReason);
                });
                it("should fail if *either* bAsset is ZERO", async () => {
                    const { bAssets, mAsset } = details;
                    const realBasset = bAssets[0];
                    const fakeBasset = constants_1.ZERO_ADDRESS;
                    const recipient = sa.default.address;
                    const expectedReason = "Invalid asset";
                    await chai_1.expect(mAsset.swap(realBasset.address, fakeBasset, 1, 0, recipient)).to.revertedWith(expectedReason);
                    await chai_1.expect(mAsset.getSwapOutput(realBasset.address, fakeBasset, 1)).to.revertedWith(expectedReason);
                    await chai_1.expect(mAsset.swap(fakeBasset, realBasset.address, 1, 0, recipient)).to.revertedWith(expectedReason);
                    await chai_1.expect(mAsset.getSwapOutput(fakeBasset, realBasset.address, 1)).to.revertedWith(expectedReason);
                });
            ...
            context("using bAssets with transfer fees", async () => {
                context("when no lending market", async () => {
                    ...
                    context("and input has xfer fee", () => {
                        it("should have lower input and proportionately lower output", async () => {
                            ...
                            // 2. Do the necessary approvals and make the calls
                            const approval0 = await mAssetMachine.approveMasset(inputBasset, mAsset, swapQuantity, sa.default.signer);
                            ...
                            // 3. Calculate expected responses
                            const inputQuantityExact = math_1.simpleToExactAmount(swapQuantity, await inputBasset.decimals());
                            const scaledInputQuantity = math_1.simpleToExactAmount(swapQuantity, 18);
                            const expectedOutputValue = scaledInputQuantity.mul(constants_1.ratioScale).div(outputBassetBefore.bData.ratio);
                            const { swapFee: feeRate } = await mAsset.data();
                            const fee = expectedOutputValue.mul(feeRate).div(constants_1.fullScale);
                            //  Input
                            //    Sender should have less input bAsset after
                            const swapperBassetBalAfter = await inputBasset.balanceOf(sender);
                            chai_1.expect(swapperBassetBalAfter).eq(swapperInputBalBefore.sub(inputQuantityExact));
                            //    VaultBalance should update for input bAsset
                            const inputBassetAfter = await mAsset.getBasset(inputBasset.address);
                            // Assert that only >99.7 && <100% of the asset got added to the vault
                            assertions_1.assertBNSlightlyGTPercent(inputQuantityExact, math_1.BN.from(inputBassetAfter.bData.vaultBalance).sub(inputBassetBefore.bData.vaultBalance), "0.3", true);
                            //  Output
                            //    Recipient should have output asset quantity after (minus fee)
                            const recipientBalAfter = await outputAsset.balanceOf(recipient);
                            // Assert recipient only receives x amount
                            assertions_1.assertBNSlightlyGTPercent(expectedOutputValue.sub(fee), recipientBalAfter.sub(recipientOutputBalBefore), "0.3", true);
                ...
                context("when a lending market integration", async () => {
                    ...
                    context("and input has xfer fee", () => {
                        ...
                        it("should fail if the system doesn't know about the fee", async () => {
                            ...
                            await assertFailedSwap(mAsset, bAssets[3], bAssets[0], 1, 0, "Asset not fully transferred", sa.default.signer, sa.default.address, false);
                        });
                    });
                });
                describe("when output has xfer fee", async () => {
                    ...
                    it("should have same input but lower physical output", async () => {
                        ...
                        //    VaultBalance should update for input bAsset
                        const inputBassetAfter = await mAsset.getBasset(inputBasset.address);
                        chai_1.expect(math_1.BN.from(inputBassetAfter.bData.vaultBalance)).eq(math_1.BN.from(inputBassetBefore.bData.vaultBalance).add(inputQuantityExact));
                        //  Output
                        //    Recipient should have output asset quantity after (minus fee)
                        const recipientBalAfter = await outputAsset.balanceOf(recipient);
                        // Assert recipient only receives x amount
            ...
            context("with an affected bAsset", async () => {
                ...
                it("should fail if input basset has lost its peg", async () => {
                    ...
                    chai_1.expect(inputBasset.personal.status).to.eq(mstable_objects_1.BassetStatus.BrokenBelowPeg);
                    const outputBasset = await mAsset.getBasset(output.address);
                    chai_1.expect(outputBasset.personal.status).to.eq(mstable_objects_1.BassetStatus.Normal);
                ...
                it("should fail if output basset has lost its peg", async () => {
                    ...
                    const inputBasset = await mAsset.getBasset(input.address);
                    chai_1.expect(inputBasset.personal.status).to.eq(mstable_objects_1.BassetStatus.Normal);
                    const outputBasset = await mAsset.getBasset(output.address);
                    chai_1.expect(outputBasset.personal.status).to.eq(mstable_objects_1.BassetStatus.BrokenBelowPeg);
                    await assertFailedSwap(mAsset, input, output, 1, 0, "Unhealthy", undefined, undefined, true, false, 0.999003);
                });
            });
        });
