...
describe("Many asset Masset", () => {
    ...
    const runSetup = async () => {
        ...
        const factory = (await hardhat_1.ethers.getContractFactory("ExposedMasset", {
            libraries: {
        ...
        const data = impl.interface.encodeFunctionData("initialize", [
            ...
            })),
            {
                ...
                limits: {
                    ...
                    max: math_1.simpleToExactAmount(37, 16),
                },
            },
        ]);
        ...
        details = {
            mAsset: (await factory.attach(mAsset.address)),
            bAssets,
        };
    };
    ...
    describe("testing some mints", () => {
        ...
        it("should mint some bAssets", async () => {
            ...
            chai_1.expect(dataEnd.totalSupply).eq(math_1.simpleToExactAmount(500, 18));
        });
        it("should mint less when going into penalty zone", async () => {
            ...
            await mAsset["mint(address,uint256,uint256,address)"](bAssets[0].address, approval, math_1.simpleToExactAmount(89), sa.default.address);
            const dataEnd = await mAssetMachine.getBasketComposition(details);
            const minted = dataEnd.totalSupply.sub(dataBefore.totalSupply);
            chai_1.expect(minted).lt(math_1.simpleToExactAmount(90, 18));
            chai_1.expect(minted).gt(math_1.simpleToExactAmount("89.6", 18));
        });
        it("should apply close to 5% penalty near hard max", async () => {
            ...
            await mAsset["mint(address,uint256,uint256,address)"](bAssets[0].address, approval, math_1.simpleToExactAmount(37), sa.default.address);
            const dataEnd = await mAssetMachine.getBasketComposition(details);
            const minted = dataEnd.totalSupply.sub(dataBefore.totalSupply);
            chai_1.expect(minted).lt(math_1.simpleToExactAmount(40, 18));
            chai_1.expect(minted).gt(math_1.simpleToExactAmount("39.3", 18));
        });
        it("should fail if we go over max", async () => {
            const { bAssets, mAsset } = details;
            const approval = await mAssetMachine.approveMasset(bAssets[0], mAsset, 30);
            await chai_1.expect(mAsset["mint(address,uint256,uint256,address)"](bAssets[0].address, approval, math_1.simpleToExactAmount(10), sa.default.address)).to.be.revertedWith("Exceeds weight limits");
        });
        it("should allow lots of minting", async () => {
            ...
            await mAsset
                .connect(sa.dummy2.signer)["mint(address,uint256,uint256,address)"](bAssets[2].address, approval2.div(5), 0, sa.default.address);
            await mAsset
                .connect(sa.dummy2.signer)["mint(address,uint256,uint256,address)"](bAssets[2].address, approval2.div(5), 0, sa.default.address);
            await mAsset
                .connect(sa.dummy2.signer)["mint(address,uint256,uint256,address)"](bAssets[2].address, approval2.div(5), 0, sa.default.address);
    ...
    describe("testing some swaps", () => {
        ...
        before("set up basket", async () => {
            ...
            chai_1.expect(dataStart.totalSupply).eq(math_1.simpleToExactAmount(500, 18));
        });
        it("should swap almost 1:1(-fee) within normal range", async () => {
            ...
            await mAsset["swap(address,address,uint256,uint256,address)"](bAssets[0].address, bAssets[1].address, approval, math_1.simpleToExactAmount("9.95"), sa.default.address);
            ...
            chai_1.expect(dataAfter.bAssets[0].mAssetUnits.sub(dataStart.bAssets[0].mAssetUnits)).eq(math_1.simpleToExactAmount(10, 18));
            chai_1.expect(dataAfter.totalSupply).eq(dataStart.totalSupply);
        });
        it("should apply minute fee when 2% over soft max ", async () => {
            ...
            await mAsset["swap(address,address,uint256,uint256,address)"](bAssets[0].address, bAssets[2].address, approval, math_1.simpleToExactAmount(49, 12), sa.default.address);
            const dataAfter = await mAssetMachine.getBasketComposition(details);
            ...
            chai_1.expect(swappedOut).gt(math_1.simpleToExactAmount("49.6", 18));
            chai_1.expect(swappedOut).lt(math_1.simpleToExactAmount(50, 18));
        });
        it("should apply close to 5% penalty near hard max", async () => {
            ...
            await mAsset["swap(address,address,uint256,uint256,address)"](bAssets[0].address, bAssets[1].address, approval, math_1.simpleToExactAmount(22), sa.default.address);
            const dataAfter = await mAssetMachine.getBasketComposition(details);
            const swappedOut = dataBefore.bAssets[1].mAssetUnits.sub(dataAfter.bAssets[1].mAssetUnits);
            // sum of fee is 0.5% (incl 0.06% swap fee)
            chai_1.expect(swappedOut).lt(math_1.simpleToExactAmount(24, 18));
            chai_1.expect(swappedOut).gt(math_1.simpleToExactAmount("22.8", 18));
        });
        it("should fail if we go over max", async () => {
            const { bAssets, mAsset } = details;
            const approval = await mAssetMachine.approveMasset(bAssets[0], mAsset, 10);
            await chai_1.expect(mAsset["swap(address,address,uint256,uint256,address)"](bAssets[0].address, bAssets[2].address, approval, math_1.simpleToExactAmount(9, 12), sa.default.address)).to.be.revertedWith("Exceeds weight limits");
        });
    });
    describe("testing redeem exact mAsset", () => {
        ...
        it("should redeem almost 1:1(-fee) within normal range", async () => {
            ...
            await mAsset["redeem(address,uint256,uint256,address)"](bAssets[0].address, mAssetRedeemAmount, minBassetAmount, sa.default.address);
            const dataAfter = await mAssetMachine.getBasketComposition(details);
            ...
            chai_1.expect(dataAfter.totalSupply).eq(dataStart.totalSupply.sub(mAssetRedeemAmount));
        });
        it("should apply minute fee when 2% under soft min ", async () => {
            ...
            const { bAssets, mAsset } = details;
            const dataBefore = await mAssetMachine.getBasketComposition(details);
            const mAssetRedeemAmount = math_1.simpleToExactAmount(50, 18);
            const minBassetAmount = math_1.simpleToExactAmount(49, 18);
            await mAsset["redeem(address,uint256,uint256,address)"](bAssets[0].address, mAssetRedeemAmount, minBassetAmount, sa.default.address);
            const dataAfter = await mAssetMachine.getBasketComposition(details);
            ...
            chai_1.expect(redeemed).gt(math_1.simpleToExactAmount("49.6", 18));
            chai_1.expect(redeemed).lt(math_1.simpleToExactAmount("49.95", 18));
            chai_1.expect(dataAfter.totalSupply).eq(dataBefore.totalSupply.sub(mAssetRedeemAmount));
            chai_1.expect(dataAfter.surplus.sub(dataBefore.surplus)).eq(math_1.simpleToExactAmount(30, 15));
        });
        it("should apply close to 5% penalty near hard min", async () => {
            ...
            const { bAssets, mAsset } = details;
            const dataBefore = await mAssetMachine.getBasketComposition(details);
            const mAssetRedeemAmount = math_1.simpleToExactAmount(18, 18);
            const minBassetAmount = math_1.simpleToExactAmount(14, 18);
            await mAsset["redeem(address,uint256,uint256,address)"](bAssets[0].address, mAssetRedeemAmount, minBassetAmount, sa.default.address);
            const dataAfter = await mAssetMachine.getBasketComposition(details);
            const bAssetRedeemed = dataBefore.bAssets[0].mAssetUnits.sub(dataAfter.bAssets[0].mAssetUnits);
            // max slippage around 9%
            chai_1.expect(bAssetRedeemed).gt(math_1.simpleToExactAmount("16.6", 18));
            chai_1.expect(bAssetRedeemed).lt(math_1.simpleToExactAmount("17.52", 18));
            chai_1.expect(dataAfter.totalSupply).eq(dataBefore.totalSupply.sub(mAssetRedeemAmount));
        });
    });
    describe("testing redeem exact bAsset(s)", () => {
        ...
        before("set up basket", async () => {
            ...
            await mAsset.mintMulti(bAssets.map((b) => b.address), approvals, 99, sa.default.address);
            dataStart = await mAssetMachine.getBasketComposition(details);
            chai_1.expect(dataStart.totalSupply).eq(math_1.simpleToExactAmount(300, 18));
        });
    });
});
//# sourceMappingURL=large-basket.spec.js.map